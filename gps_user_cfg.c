...
void gps_user_cfg_init(void) {
	...
	// gps_config_load_from_nvs(); // Disabled - unified config system handles
	// this now
}
...
static void set_gps_time_out_msg(void) {
	FUNC_ENTRY(TAG);
	gps->time_out_gps_msg =
		HZ_TO_MS(g_rtc_config.ubx.output_rate) + 75; // max time out = 175 ms
}
...
uint8_t gps_cnf_set_item(uint8_t pos, void *el, uint8_t force) {
	...
	if (pos < CFG_GPS_ITEM_BASE ||
		pos >= CFG_GPS_ITEM_BASE + gps_user_cfg_item_count) {
		return 255;
	}
	...
	if (config_lock(timeout_max) == pdTRUE) {
		...
		case gps_cfg_file_date_time:
			...
			if (!ret) {
				char val_str[16];
				snprintf(val_str, sizeof(val_str), "%u",
						 g_rtc_config.gps.file_date_time);
				config_gps_set_item(index, val_str);
				changed = gps_cfg_file_date_time;
			}
			break;
		case gps_cfg_timezone: // choice for timedifference in hours with UTC,
							   // for Belgium 1, 2, 3 for estonia (summer time)
			ret = set_f(item, &g_rtc_config.gps.timezone, 0);
			if (!ret) {
				char val_str[16];
				snprintf(val_str, sizeof(val_str), "%.0f",
						 g_rtc_config.gps.timezone);
				config_gps_set_item(index, val_str);
				changed = gps_cfg_timezone;
			}
			break;
		case gps_cfg_stat_screens: // choice for stats field when no speed, here
								   // stat_screen 1, 2 and 3 will be active
			ret = set_u(item, &g_rtc_config.gps.stat_screens, 0);
			if (!ret) {
				char val_str[16];
				snprintf(val_str, sizeof(val_str), "%u",
						 g_rtc_config.gps.stat_screens);
				config_gps_set_item(index, val_str);
		...
		case gps_cfg_gnss: // default setting 2 GNSS, GPS & GLONAS
			...
			break;
		// case gps_cfg_dynamic_model_auto: /// wether device can switch mode
		// automatically
		...
		case gps_cfg_dynamic_model: // sea, portable, automotive, pedestrian
			...
			break;
		case gps_cfg_log_txt: // || !strcmp(var, "logTXT")) {  // switchinf off
							  // .txt files
			ret = set_bit(item, &log_config.log_file_enables.value, SD_TXT, 0);
			if (!ret) {
				bool val = (log_config.log_file_enables.value & (1 << SD_TXT))
							   ? true
							   : false;
		...
		case gps_cfg_log_ubx: // || !strcmp(var, "logUBX")) {  // log to .ubx
			...
			if (!ret) {
				bool val = (log_config.log_file_enables.value & (1 << SD_UBX))
							   ? true
							   : false;
				config_gps_set_item(index, &val);
			...
			goto test_if_log_set; // if no log file selected, set SBP as default
		case gps_cfg_log_ubx_nav_sat: // || !strcmp(var, "logUBX_nav_sat")) { //
									  // log nav sat msg to .ubx
		...
		case gps_cfg_log_sbp: // || !strcmp(var, "logSBP")) {  // log to .sbp
			...
			if (!ret) {
				bool val = (log_config.log_file_enables.value & (1 << SD_SBP))
							   ? true
							   : false;
				config_gps_set_item(index, &val);
		...
		test_if_log_set:
			if (!gps_log_file_bits_check(&log_config.log_file_enables))
				log_config.log_file_enables.bits.log_sbp =
					1; // set SBP if none selected
			break;
		case gps_cfg_log_gpx: //  || !strcmp(var, "logGPX")) {
			ret = set_bit(item, &log_config.log_file_enables.value, SD_GPX, 0);
			if (!ret) {
				bool val = (log_config.log_file_enables.value & (1 << SD_GPX))
							   ? true
							   : false;
				config_gps_set_item(index, &val);
		...
		case gps_cfg_log_gpy: // || !strcmp(var, "logGPY")) {
			...
			if (!ret) {
				bool val = (log_config.log_file_enables.value & (1 << SD_GPY))
							   ? true
							   : false;
				config_gps_set_item(index, &val);
	...
	if (changed < 253) {
		esp_event_post(GPS_LOG_EVENT, GPS_LOG_EVENT_CFG_CHANGED, &changed,
					   sizeof(changed), timeout_max);
	}
	return changed;
}
...
esp_err_t gps_config_decode(const char *json) {
	...
	if (g_rtc_config.gps.file_date_time == 0 &&
		GETBIT(log_config.log_file_bits, SD_TXT) == 0)
		SETBIT(log_config.log_file_bits,
			   SD_TXT); // because txt file is needed for generating new file
						// count !!
#if defined(CONFIG_GPS_LOG_USE_CJSON)
	cJSON_Delete(root);
#else
...
static const char *const cfg_values[] = {
	...
	",\"info\":\"choice for dynamic model: 'Portable' (max 310m/s), 'Sea' (max "
	"25m/s) and 'Automotive' (max 100m/s) deviation medium, 'Pedestrian' (max "
	"30m/s) deviation small\",\"type\":\"int\"",
	",\"info\":\"log to ",
	"\",\"type\":\"bool\"",
	",\"info\":\"log nav sat msg to .ubx\", \"depends\":\"",
	",\"info\":\"timezone: The local time difference in hours with "
	"UTC\",\"type\":\"float\",\"ext\":\"h\"",
	",\"info\":\"type of filenaming, with MAC adress or "
	"datetime\",\"type\":\"int\"",
	",\"info\":\"your preferred filename base\",\"type\":\"str\"",
	",\"info\":\"Wether device can switch dynamic model automatically. When "
	"disabled make sure dynamic model is in your expected speed limits.",
};

static uint8_t add_from_list(strbf_t *lsb, const char *const *list,
							 size_t len) {
...
uint8_t gps_cnf_get_item(uint8_t pos, strbf_t *lsb, uint8_t mode) {
	...
	if (config_lock(timeout_max) == pdTRUE) {
		...
		case gps_cfg_file_date_time:
			...
			if (mode) {
				...
				add_from_list(lsb, file_date_time_items,
							  file_date_time_items_count);
			}
			break;
		...
		case gps_cfg_stat_screens: // choice for stats field when no speed, here
			...
			if (mode) {
				strbf_puts(lsb,
						   ",\"info\":\"Stat_screens choice : activate / "
						   "deactivate screens to show.\",\"type\":\"int\"");
				strbf_puts(lsb, ",\"toggles\":[");
				uint16_t j = 1;
				for (uint8_t i = 0, k = gps_stat_screen_item_count; i < k;
					 i++, j <<= 1) {
					strbf_puts(lsb, "{\"pos\":");
					strbf_putn(lsb, i);
					strbf_puts(lsb, cfg_values[2]);
		...
		case gps_cfg_sample_rate: // gps_rate in Hz, 1, 5 or 10Hz !!!
			...
			if (mode) {
				...
				for (uint8_t i = 0, j = sample_rates_items_count; i < j; i++) {
					strbf_puts(lsb, cfg_values[1]);
					strbf_putn(lsb, i == 0	 ? UBX_OUTPUT_1HZ
									: i == 1 ? UBX_OUTPUT_2HZ
									: i == 2 ? UBX_OUTPUT_5HZ
									: i == 3 ? UBX_OUTPUT_10HZ
									: i == 4 ? UBX_OUTPUT_16HZ
		...
		case gps_cfg_gnss:
			...
			if (mode) {
				...
				for (uint8_t i = 0, j = gnss_desc_items_count; i < j; i++) {
					if (gps && g_rtc_config.ubx.hw_type < UBX_TYPE_M9 && i == 0)
						continue;
					strbf_puts(lsb, cfg_values[1]);
		...
		case gps_cfg_dynamic_model:
			...
			if (mode) {
				...
				strbf_puts(lsb, cfg_values[0]);
				for (uint8_t i = 0, j = dynamic_models_items_count; i < j;
					 i++) {
					strbf_puts(lsb, cfg_values[1]);
			...
			break;
		// case gps_cfg_dynamic_model_auto: // )])||!strcmp(name, "logTXT")) {
		// // switchinf off .txt files
		...
		//     }
		//     break;
		case gps_cfg_log_txt: // )])||!strcmp(name, "logTXT")) {  // switchinf
							  // off .txt files
			strbf_putc(
				lsb, GETBIT(log_config.log_file_bits, SD_TXT) == 1 ? '1' : '0');
			if (mode) {
		...
		case gps_cfg_log_ubx: // )])||!strcmp(name, "logUBX")) {  // log to .ubx
			strbf_putc(
				lsb, GETBIT(log_config.log_file_bits, SD_UBX) == 0 ? '0' : '1');
			if (mode) {
				strbf_puts(lsb, cfg_values[6]); // ",\"info\":\"log to"
			...
			break;
		case gps_cfg_log_ubx_nav_sat: // )])||!strcmp(name, "logUBX_nav_sat")) {
									  // // log nav sat msg to .ubx
			strbf_putn(lsb, rtc_config.msgout_sat);
			if (mode) {
				strbf_puts(
					lsb,
					cfg_values[8]); // ",\"info\":\"log nav sat msg to .ubx"
				strbf_puts(
					lsb,
					config_gps_items[gps_cfg_log_ubx]); // depends on logUBX
				strbf_puts(lsb, cfg_values[7]); // ",\"type\":\"bool\"");
			}
			break;
		case gps_cfg_log_sbp: // )])||!strcmp(name, "logSBP")) {  // log to .sbp
			strbf_putc(
				lsb, GETBIT(log_config.log_file_bits, SD_SBP) == 0 ? '0' : '1');
			if (mode) {
				strbf_puts(lsb, cfg_values[6]); // ",\"info\":\"log to"
			...
			break;
		case gps_cfg_log_gpx: // )])||!strcmp(name, "logGPX")) {   // log to
							  // .gpx
			strbf_putc(
				lsb, GETBIT(log_config.log_file_bits, SD_GPX) == 0 ? '0' : '1');
			if (mode) {
				strbf_puts(lsb, cfg_values[6]); // ",\"info\":\"log to"
			...
			break;
#ifdef GPS_LOG_ENABLE_GPY
		case gps_cfg_log_gpy: // )])||!strcmp(name, "logGPY")) {   // log to
							  // .gpy
			strbf_putc(
				lsb, GETBIT(log_config.log_file_bits, SD_GPY) == 0 ? '0' : '1');
			if (mode) {
				strbf_puts(lsb, cfg_values[6]); // ",\"info\":\"log to"
	...
	DLOG(TAG, "[%s] conf: %s len: %d", __func__, strbf_finish(lsb),
		 lsb->cur - lsb->start);
	return pos;
}
