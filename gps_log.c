...
void gps_log_print_stats(uint32_t period_ms, uint8_t expected_hz) {
    ...
    period_msg_stats.count_nav_pvt = cur_msg_stats.count_nav_pvt - prev_msg_stats.count_nav_pvt;  // Valid PVTs processed
    period_msg_stats.count_nav_dop = cur_msg_stats.count_nav_dop - prev_msg_stats.count_nav_dop;  // DOPs processed
    period_msg_stats.count_ok = cur_msg_stats.count_ok - prev_msg_stats.count_ok;    // File push successes
    period_msg_stats.count_err = cur_msg_stats.count_err - prev_msg_stats.count_err;  // File push failures
    period_msg_stats.count = cur_msg_stats.count_nav_pvt + cur_msg_stats.count_nav_dop;  // Total valid messages processed

    float period_s = (float)period_ms / 1000.0f;
    float throughput = period_s > 0.0f ? (float)period_msg_stats.count / period_s : 0.0f;
    float expected_count = (expected_hz * (float)period_s)*2+1;  // Total messages expected in this period
    float loss_pct = expected_count > 0.0f ? (1.0f - (float)period_msg_stats.count / expected_count) * 100.0f : 0.0f;
    if (loss_pct < 0.0f) loss_pct = 0.0f;  // Clamp negative loss (happens when rate > expected)

    printf("[GPS] ========== GPS LOG STATS ==========\n");
    printf("[GPS] GPS messages: %.1f msg/s (expected:  %.1f msg/s at %hhu Hz, loss: %.1f%%)\n",
        throughput, expected_count, expected_hz, loss_pct);
    printf("[GPS] Valid messages (period): PVT=%"PRIu32" DOP=%"PRIu32" (passed GPS validity checks)\n", 
        period_msg_stats.count_nav_pvt, period_msg_stats.count_nav_dop);
    printf("[GPS] File I/O (period): ok=%"PRIu32" fail=%"PRIu32" (%.1f%% loss)\n", 
        period_msg_stats.count_ok, period_msg_stats.count_err,
        (period_msg_stats.count_ok + period_msg_stats.count_err) > 0 ? 
            (float)period_msg_stats.count_err * 100.0f / (float)(period_msg_stats.count_ok + period_msg_stats.count_err) : 0.0f);
    printf("[GPS] Totals: Valid PVT=%"PRIu32" DOP=%"PRIu32" | File ok=%"PRIu32" err=%"PRIu32"\n", 
...
void gps_log_print_all_stats(void* arg) {
    ...
    printf("\n");
    if(arg)
        ubx_uart_print_buffer_stats((struct ubx_ctx_s *)arg); // UART buffer health
...
static log_context_t lctx = {.old_run_count = 0, .gps_log_delay = 0, .old_nav_pvt_itow = 0, .next_time_sync = 0, .ubx_restart_requested = false, 
...
// Adjust GPS output rate based on available partition space (event-driven from partition change, not hot loop)
// Called when: partition changes, UBX device initializes, or partition space is updated
static void gps_adjust_rate_for_partition_space(void) {
    ...
    if (!lctx.output_rate_swp && g_rtc_config.ubx.output_rate >= UBX_OUTPUT_5HZ && 
        free_bytes < TO_K_UL(700)) {
        WLOG(TAG, "[%s] vfs log part %s free space too low: %llu! Dropping rate.", 
             __FUNCTION__, vfs_ctx.parts[vfs_ctx.gps_log_part].mount_point, free_bytes);
        lctx.output_rate_swp = g_rtc_config.ubx.output_rate;  // Save current rate
    ...
    else if (lctx.output_rate_swp && g_rtc_config.ubx.output_rate < UBX_OUTPUT_5HZ && 
             free_bytes >= TO_K_UL(700)) {
        WLOG(TAG, "[%s] vfs log part %s free space ok again: %llu! Restoring rate.", 
             __FUNCTION__, vfs_ctx.parts[vfs_ctx.gps_log_part].mount_point, free_bytes);
        // Restore to one step above saved rate (hysteresis to avoid oscillation)
...
static void gps_vfs_processor(vfs_work_type_t type, void *arg) {
    ...
    switch(type) {
        ...
        case VFS_WORK_PARTITION_CHANGED:
            // Partition changed: evaluate rate adjustment based on new partition's free space
...
static esp_err_t set_time(nav_pvt_t *nav_pvt, float time_offset) {
    ...
    if (!nav_pvt || !nav_pvt->numSV) return ESP_FAIL;
    if (nav_pvt->year < 2023) {
        WLOG(TAG, "[%s] invalid time data: %hu-%hhu-%hhu", __func__, nav_pvt->year, nav_pvt->month, nav_pvt->day);
        return ESP_FAIL;
    }
    ...
    struct tm my_time = {
        ...
        .tm_year = nav_pvt->year - 1900,    // mktime needs years since 1900, so deduct 1900
        .tm_isdst = -1,                 // daylight saving time flag
    };
    int ret = c_set_time(&my_time, NANO_TO_US_ROUND(nav_pvt->nano), time_offset);  // set the time in the struct tm
    if (ret) {
    ...
    // int64_t utc_ms = unix_timestamp * 1000LL + NANO_TO_MILLIS_ROUND(nav_pvt->nano);
    // int64_t time_us = (int64_t)tv_now.tv_sec * 1000000L + (int64_t)tv_now.tv_usec;
    FUNC_ENTRY_ARGSD(TAG, "GPS pvt time: %d-%02d-%02d %02d:%02d:%02d %ld %hhu, offset: %f", nav_pvt->year, nav_pvt->month, nav_pvt->day, nav_pvt->hour, nav_pvt->minute, nav_pvt->second, nav_pvt->nano, nav_pvt->id, time_offset);
    // WLOG(TAG, "GPS tm time : %d-%02d-%02d %02d:%02d:%02d %ld", my_time.tm_year+1900, my_time.tm_mon+1, my_time.tm_mday, my_time.tm_hour, my_time.tm_min, my_time.tm_sec, NANO_TO_US_ROUND(nav_pvt->nano));
#if (C_LOG_LEVEL <= LOG_INFO_NUM)
    struct tm tm;
    get_local_time(&tm);
    FUNC_ENTRY_ARGS(TAG, "GPS time set: %d-%02d-%02d %02d:%02d:%02d", (tm.tm_year) + 1900, (tm.tm_mon) + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
#endif
...
static void gpsTask(void *parameter) {
    ...
    while (lctx.gps_task_is_running) {
        now = get_millis();
        uint8_t has_decoded_count = 0;  // Track messages decoded this iteration for adaptive yielding
#if (C_LOG_LEVEL <= LOG_DEBUG_NUM || GPS_TASK_DEBUG > 1)
        // Debug: verify loop is running
        if ((loops > 0) && (loops % 500) == 0) {
            DLOG(TAG, "LOOP_ALIVE: l=%lu now=%lu", loops, now);
        }
#endif
        if (!gps_has_version_set() || lctx.ubx_restart_requested) {
            ...
            if(lctx.ubx_fail_count>50) {
                if(!gps_has_version_set()) { // only when no hwVersion is received
                    ...
                    WLOG(TAG, "[%s] Gps init failed, restart requested.", __FUNCTION__);
                }
                else
...
#if (C_LOG_LEVEL <= LOG_DEBUG_NUM || defined(GPS_TASK_DEBUG))
            if ((loops % 100) == 0) {
                WLOG(TAG, "gps not ready, next loop l=%lu", loops);
            }
#endif
            goto loop_tail;
        }
        else {
            // UBX device is ready: evaluate rate adjustment when partition is available
            static bool ubx_init_rate_adjusted = false;
            if (!ubx_init_rate_adjusted && gps_has_version_set()) {
                // First time UBX becomes ready: check rate constraints based on partition space
                gps_adjust_rate_for_partition_space();
        ...
        // CRITICAL: Copy NAV_PVT data immediately to protect from buffer overwrites
        // The shared buffer (ubx_ctx->ubx_msg) is reused for every decoded message
        nav_pvt_t pvt_snapshot;  // Local copy of current NAV_PVT (protects from overwrites)
        bool had_nav_dop = false;  // Track NAV_DOP for first fix detection
        int iterations = 0;
        
        // Process all pending messages (event-driven: decode → process → complete)
        while (true) {
            ...
            if (!ret) {
                if(!has_decoded_count) has_decoded_count++;
                lctx.ubx_fail_count = 0;
                
                // Event-driven: Process each message type immediately
                switch(ubx_packet.ubx_msg_type) {
                    case MT_NAV_PVT:
                        // CRITICAL: Copy NAV_PVT data FIRST to protect from subsequent overwrites
                        // ubx_msg_handler() writes directly into shared buffer, so next decode will overwrite this!
                        memcpy(&pvt_snapshot, &ubxMessage->navPvt, sizeof(nav_pvt_t));
                        ...
                        // Check for first fix (needs DOP data from previous message)
                        if (had_nav_dop) {
                            if ((pvt_snapshot.numSV >= MIN_numSV_FIRST_FIX) && (MS_TO_SEC(pvt_snapshot.sAcc) < MAX_Sacc_FIRST_FIX) && 
                                (pvt_snapshot.valid >= 7) && (!gps->signal_ok && !ubx_ctx->shutdown_requested)) {
                                gps->signal_ok = true;
                        ...
                        if (!gps->files_opened && gps->signal_ok && 
                            (lctx.gps_log_delay > (TIME_DELAY_FIRST_FIX * g_rtc_config.ubx.output_rate))) {
                        ...
                        // Process speed/movement immediately using LOCAL snapshot (safe from overwrites)
                        if (gps->time_set && log_p_lctx.count_nav_pvt > 10) {
                            ...
                            if ((pvt_snapshot.numSV <= MIN_numSV_GPS_SPEED_OK) || 
                                ...
                                FUNC_ENTRY_ARGW(TAG, "GPS REJECTED: sats=%hhu acc=%lumm speed=%ldmm/s",
                                ...
                                // Count only PVTs that passed GPS validity checks (not rejected)
                                ++cur_msg_stats.count_nav_pvt;
                            }
#endif
                            
                            // Movement detection and logging (immediate while data fresh)
                            if (gps->gps_speed > STANDSTILL_DETECTION_MAX) {
                                // // CRITICAL: Update nav_pvt pointer to local snapshot before logging
                                // // log_to_file() and other functions access nav_pvt via pointer
                                // struct nav_pvt_s *orig_pvt = nav_pvt;
                                // nav_pvt = &pvt_snapshot;
                                
                                // Log to file immediately using protected snapshot
                                if (gps->files_opened) {
                                    log_to_file(gps);
                                    uint32_t time_diff_ms = pvt_snapshot.iTOW - lctx.old_nav_pvt_itow;  // Correct order
                                    uint32_t threshold_ms = HZ_TO_MS(g_rtc_config.ubx.output_rate);  // Frame length
                                    if(gps->gps_speed > 2000) { // only check timeouts when speed > 2 m/s
                                        
                                        if (time_diff_ms > (threshold_ms * 10)) { // 10 x frame length
                                            gps->gps_timeout_flag++;
                                            gps->lost_frames++;
                                            if(gps->gps_timeout_flag==1)
                                                log_gps_timeout(gps, time_diff_ms, "Timeout");
                                        }
                                        else if (time_diff_ms > threshold_ms) {
                                            gps->frame_lost_flag++;
                                            log_gps_timeout(gps, time_diff_ms, "Lost frame");
                                        }
                                    }
                                }
...
#if (C_LOG_LEVEL <= LOG_INFO_NUM || defined(GPS_TASK_DEBUG))
                                    FUNC_ENTRY_ARGW(TAG, "*** GPS IS MOVING *** speed=%ldmm/s", gps->gps_speed);
#endif
                                ...
                                if (gps->gps_is_moving) {
#if (C_LOG_LEVEL <= LOG_INFO_NUM || defined(GPS_TASK_DEBUG))
                                    FUNC_ENTRY_ARGW(TAG, "*** GPS IS STOPPING *** speed=%ldmm/s", gps->gps_speed);
#endif
                            ...
                            // Speed metrics and run detection using protected snapshot
                            esp_err_t ret = push_gps_data(gps, &gps->Ublox, FROM_10M(pvt_snapshot.lat), 
                                                         FROM_10M(pvt_snapshot.lon), gps->gps_speed);
                            if (!ret) {
                                new_run_detection(gps, FROM_100K(pvt_snapshot.heading), time_cur_speed(time_2s));
...
#if (C_LOG_LEVEL <= LOG_INFO_NUM || defined(GPS_TASK_DEBUG))
                                    FUNC_ENTRY_ARGW(TAG, "*** New run *** speed=%ldmm/s", gps->gps_speed);
#endif
                                }
                ...
                // Decode error
                if ((gps_has_version_set() && (ret == ESP_ERR_TIMEOUT && lctx.ubx_fail_count > 3)) || 
        ...
        // After processing all messages: handle time sync if needed (check timing once here)
        if (now >= lctx.next_time_sync || lctx.next_time_sync == 0) {
            if (set_time(&pvt_snapshot, g_rtc_config.gps.timezone) == ESP_OK) { 
                lctx.next_time_sync = now + SEC_TO_MS(30); // Sync again in 30 seconds
            } else {
                lctx.next_time_sync = now + SEC_TO_MS(5);  // Retry in 5 seconds on failure
            }
        }
    ...
    loop_tail:
        // Loop-tail yield: cooperative only; main delay is applied after Phase 1 when messages are decoded
        vTaskDelay(0);
        
#if (C_LOG_LEVEL <= LOG_DEBUG_NUM || defined(GPS_TASK_DEBUG))
        loops++;
        if ((loops % 100) == 0) {
            ILOG(TAG, "BEAT: l=%lu r=%d t=%d s=%d p=%lu sv=%hhu",
                loops, ubx_ctx->ready, gps->time_set, gps->signal_ok, log_p_lctx.count_nav_pvt, pvt_snapshot.numSV);
        }
#endif
        continue;
    }
