...
uint32_t buffer_caps = MALLOC_CAP_INTERNAL;
#else
uint32_t buffer_caps = MALLOC_CAP_8BIT;
#endif
...
void unalloc_buffer(void **buf) {
	if (buf && *buf) {
		...
		FUNC_ENTRY_ARGW(TAG, "Unallocated buffer");
	}
}
...
static size_t calculate_safe_allocation_size(size_t requested_bytes, uint32_t caps,
                                            size_t min_bytes) {
    const float HEAP_SAFETY_FACTOR = 0.7f;
...
bool check_and_alloc_buffer(void **buf, size_t required_count, size_t elem_size,
	...
	FUNC_ENTRY_ARGS(TAG, "required_count: %zu, elem_size: %zu, caps : %s", required_count, elem_size, caps == MALLOC_CAP_INTERNAL ? "INTERNAL" : "DEFAULT");
	if (!buf)
		return false;
	...
	if (*buf && current_count && *current_count >= required_count) {
		ESP_LOGI(TAG, "Buffer reused: current=%" PRIu16 " >= required=%zu", *current_count, required_count);
		return true;
	}
	// Free existing buffer BEFORE calculating safe size so its memory is
	// included in heap availability (avoids underestimating free heap)
	unalloc_buffer(buf);
	...
	ESP_LOGI(TAG, "Allocating %zu bytes (safe: %zu, heap: free=%zu largest=%zu)",
		 alloc_size, safe_alloc_size, free_heap, largest_block);
	...
	*buf = heap_caps_malloc(alloc_size, caps);
	if (*buf && current_count) {
		*current_count = required_count;
		ESP_LOGI(TAG, "Allocated of size %zu, elements: %" PRIu16, alloc_size, *current_count);
		return true;
	} else {
		ESP_LOGE(TAG, "ALLOC FAILED: size=%zu free=%zu largest=%zu",
			 alloc_size, heap_caps_get_free_size(caps),
...
void gps_check_alfa_buf(size_t new_size) {
	FUNC_ENTRY_ARGS(TAG, "Checking alfa buffer for new size: %zu", new_size);
	...
	if (log_p_lctx.alfa_buf) {
		...
		if (log_p_lctx.alfa_buf_size < new_size) {
			...
			FUNC_ENTRY_ARGW(TAG, "Reduced alfa buffer: %zu→%" PRIu16 " elements (%.0fm range requires ≥%.1f km/h, was ≥%.1f km/h)",
				 new_size, log_p_lctx.alfa_buf_size, ALFA_DISTANCE_MAX, reduced_speed_kmh, original_speed_kmh);
		}
#endif
		ESP_LOGI(TAG,
			 "[%s] alfa buffer resized: requested=%zu elems (%zu bytes), "
			 "allocated=%" PRIu16 " elems (%zu bytes), rate=%d Hz",
		...
		FUNC_ENTRY_ARGW(TAG, "Failed to allocate alfa buffer of size %zu", new_size);
	}
#endif
}
#endif
...
void gps_check_sec_buf(size_t new_size) {
	FUNC_ENTRY_ARGS(TAG, "Checking sec speed buffer for new size: %zu", new_size);
	check_and_alloc_buffer((void **)&log_p_lctx.buf_sec_speed, new_size,
						   sizeof(int16_t), &log_p_lctx.buf_sec_speed_size,
						   buffer_caps
						   );
	// Ensure the buffer can hold at least 48 speed values (for 1 second at
	// 10Hz) and not exceed the maximum size for uint16_t.
	...
	if (log_p_lctx.buf_sec_speed) {
		...
		WLOG(TAG, "[%s] Failed to allocate sec speed buffer of size %zu",
			 __func__, new_size);
	}
#endif
}
...
static void s(void *arg) {
	...
	printf("--- GPS periodic timer callback %" PRIu32 " ---\n", get_millis());
...
static esp_err_t gps_p_context_printf(const gps_p_context_t *me) {
	...
	printf("sec_gSpeed: %" PRId32 ", ", me->sec_gSpeed);
	printf("run_count: %" PRIu32 ", ", me->run_count);
	printf("alfa_count: %" PRIu32 ", ", me->alfa_count);
	printf("old_alfa_count: %" PRIu32 "\n", me->old_alfa_count);
	...
	printf("P1: {lat: %.02f, long: %.02f}\n", me->alfa_p1.latitude,
		   me->alfa_p1.longitude);
	printf("P2: {lat: %.02f, long: %.02f}\n", me->alfa_p2.latitude,
		   me->alfa_p2.longitude);
	printf("index_gspeed: %" PRIu32 ", ", me->index_gspeed);
	printf("index_sec: %" PRIu32 "\n", me->index_sec);
	printf("buf_gspeed: ");
	uint8_t i, j = buf_index(me->index_gspeed);
	if (j < 10)
		j = 10;
	for (i = j - 10; i < j; i++)
		printf(" %" PRId32 "", me->buf_gspeed[i]);
	printf("\n");
	...
	for (i = j - 10; i < j; i++)
		printf(" %" PRIu16 "", me->buf_sec_speed[i]);
	printf("\n");
...
int32_t gps_last_speed_smoothed(uint8_t window_size) {
	return smooth(log_p_lctx.buf_gspeed, log_p_lctx.index_gspeed,
				  log_p_lctx.buf_gspeed_size, window_size);
}

int32_t gps_last_sec_speed_smoothed(uint8_t window_size) {
	return smooth((int32_t *)log_p_lctx.buf_sec_speed, log_p_lctx.index_sec,
				  log_p_lctx.buf_sec_speed_size, window_size);
}
...
void init_gps_context_fields(gps_context_t *ctx) {
	...
	if (!ctx)
		return;
	if (ctx->gps_fields_initialized)
		return;
	init_gps_data(&ctx->Ublox); // create an object storing GPS_data !
	init_gps_sat_info(
		&ctx->Ublox_Sat); // create an object storing GPS_SAT info !
...
static esp_err_t gps_data_printf(struct gps_data_s *me) {
	printf("gps_data:{ ");
	printf("run_start_time: %" PRIu32 ", ", me->run_start_time);
...
void gps_log_nav_mode_change(gps_context_t *context, uint8_t changed) {
	FUNC_ENTRY_ARGS(TAG, "%" PRIu8 "", changed);
...
const float speed_thresholds_for_alfa[] = ALFA_THRESHOLDS_MS;
const uint8_t speed_threshold_index[17] = ALFA_THRESHOLD_IDX_TABLE;

// rate counted speed buffer
static inline void update_speed_buffer(gps_point_t *p, int32_t gSpeed) {
	if (log_p_lctx.index_gspeed == UINT32_MAX)
		log_p_lctx.index_gspeed = 0;
	else
		log_p_lctx.index_gspeed++;
...
#endif
	log_p_lctx.alfa_buf[al_buf_index(log_p_lctx.index_gspeed)].latitude =
		p->latitude;
	log_p_lctx.alfa_buf[al_buf_index(log_p_lctx.index_gspeed)].longitude =
		p->longitude;
}

static inline void update_sec_speed_buffer(int32_t gSpeed,
										   uint8_t sample_rate) {
	if (log_p_lctx.index_sec == UINT32_MAX)
		log_p_lctx.index_sec = 0;
	else
		log_p_lctx.sec_gSpeed += gSpeed;
	if (log_p_lctx.index_gspeed % sample_rate == 0) {
		log_p_lctx.index_sec++;
		log_p_lctx.buf_sec_speed[sec_buf_index(log_p_lctx.index_sec)] =
			log_p_lctx.sec_gSpeed / sample_rate;
		log_p_lctx.sec_gSpeed = 0;
	}
}

int get_cur_nav_mode(int nav_mode) {
	return (log_p_lctx.dynamic_state == 1 && nav_mode == UBX_MODE_PEDESTRIAN)
			   ? UBX_MODE_PORTABLE
		   : log_p_lctx.dynamic_state == 1 && nav_mode == UBX_MODE_SEA
			   ? UBX_MODE_PORTABLE
		   : log_p_lctx.dynamic_state == 1 && nav_mode == UBX_MODE_PORTABLE
			   ? UBX_MODE_PEDESTRIAN
			   : nav_mode;
}
...
esp_err_t push_gps_data(gps_context_t *context, struct gps_data_s *me,
						float latitude, float longitude,
						int32_t gSpeed) { // gspeed in mm/s !!!
	if (!context)
	...
	if (log_p_lctx.dynamic_state == 0) {
		/// switch to dynamic_model "portable" as "pedestrian" only works with
		/// speed < 30 m/s (108kmh) and "sea" only works with speed < 25 m/s
		/// (90kmh)
		if (((spd2s > 29500 &&
			  g_rtc_config.ubx.nav_mode ==
				  UBX_MODE_PEDESTRIAN) /// change over 28ms 100kmh
			 ||
			 (spd2s > 23750 && g_rtc_config.ubx.nav_mode ==
								   UBX_MODE_SEA))) { /// change over 24ms 85kmh
			log_p_lctx.dynamic_state = 1;
			// ILOG(TAG, "Switching to UBX_MODE_PORTABLE due to speed: %.2f
			// m/s", spd2s / 1000.0f);
			g_rtc_config.ubx.nav_mode = UBX_MODE_PORTABLE;
		} else if (spd2s < 28500 &&
				   g_rtc_config.ubx.nav_mode ==
					   UBX_MODE_PORTABLE) { /// change under 28ms 100kmh
			/// switch to dynamic_model "pedestrian" below 30ms as "portable" is
			/// less accurate
			log_p_lctx.dynamic_state = 1;
			// ILOG(TAG, "Switching to UBX_MODE_PEDESTRIAN due to speed: %.2f
			// m/s", spd2s / 1000.0f);
			g_rtc_config.ubx.nav_mode = UBX_MODE_PEDESTRIAN;
		}
		if (log_p_lctx.dynamic_state == 1)
			changed = 1;
	} else if (log_p_lctx.dynamic_state == 1) {
		/// switch back to "pedestrian" below 27ms as "pedestrian" is more
		/// accurate
		if (spd2s < 27500 &&
			(g_rtc_config.ubx.nav_mode ==
			 UBX_MODE_PEDESTRIAN)) { /// change below 24ms 86kmh
			...
			g_rtc_config.ubx.nav_mode = UBX_MODE_PEDESTRIAN;
				   ...
				   (g_rtc_config.ubx.nav_mode ==
					UBX_MODE_SEA)) { /// change below 24ms 86kmh
			log_p_lctx.dynamic_state = 0;
			// ILOG(TAG, "Switching to UBX_MODE_SEA due to speed: %.2f m/s",
			// spd2s / 1000.0f);
			g_rtc_config.ubx.nav_mode = UBX_MODE_SEA;
		} else if (spd2s > 29500 &&
				   g_rtc_config.ubx.nav_mode ==
					   UBX_MODE_PORTABLE) { /// change above 28ms 100kmh
			log_p_lctx.dynamic_state = 0;
			// ILOG(TAG, "Switching to UBX_MODE_PORTABLE due to speed: %.2f
			// m/s", spd2s / 1000.0f);
			g_rtc_config.ubx.nav_mode = UBX_MODE_PORTABLE;
	...
	if (changed) {
		if (esp_event_post(GPS_LOG_EVENT,
						   GPS_LOG_EVENT_GPS_NAV_MODE_CHANGED, 0, 0,
						   pdMS_TO_TICKS(100)) != ESP_OK) {
			WLOG(TAG, "EVT_FAIL: GPS_NAV_MODE_CHANGED");
		}
	}
	// }
	...
	if ((ubxMessage->navPvt.numSV >= FILTER_MIN_SATS) &&
		(MM_TO_M(ubxMessage->navPvt.sAcc) < FILTER_MAX_sACC)) {
		log_p_lctx.delta_dist =
			(float)gSpeed / sample_rate; // convert speed to distance !!!
	...
	WLOG(TAG, "-- gSpeed: %" PRIu32 ", sAcc: %" PRIu32 ", numSv: %" PRIu8 " --",
		 gSpeed, ubxMessage->navPvt.sAcc, ubxMessage->navPvt.numSV);
#endif
	return ESP_OK;
}
